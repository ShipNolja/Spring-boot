...
public class FishingInfoPermitController {
    ...
    */
    ...
    public List<ResFishingInfoDto> simpleInfoList(@ApiParam(value = "페이지 번호",required = true) @RequestParam int page,
                                                  ...
                                                  @ApiParam(value = "정렬 기준 : area , detaileArea, port, shipName, target, startDate, endDate",required = true) @RequestParam String sortBy,
                                                  ...
                                                  @ApiParam(value = "출항 시간 : xx:xx:xx",required = true)
                                                      @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)@RequestParam(required = false) LocalDateTime startDate,
                                                  @ApiParam(value = "입항 시간 : xx:xx:xx",required = true)
                                                      @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)@RequestParam(required = false) LocalDateTime endDate) {

        return fishingInfoService.simpleInfoList(page, sortMethod, sortBy, area, detailArea, port, shipName, target, startDate, endDate);
    }
    ...
    public ResFishingInfoDto detailsInfoList(Pageable pageable, int page) {
        ...
        return fishingInfoService.detailsInfoList(pageable, page);
    }
